...
namespace OctaEventos.Application
{
    ...
    {   
        ...
        public async Task AddRedeSocial(int Id, RedeSocialDto model, bool evento)
        {
            try
            {
        ...
        public async Task<RedeSocialDto[]> SaveRedeSocialByEvento(int eventoId, RedeSocialDto[] models)
        {
            try
            {
                var redesSocial = await _redeSocialPersist.GetAllByEventoIdAsync(eventoId);
                ...
                foreach (var model in models)
                {
                    ...
                    {
                        var redeSocial = redesSocial.FirstOrDefault(redeSocial => redeSocial.Id == model.Id);
        ...
        public async Task<RedeSocialDto[]> SaveRedeSocialByPalestrante(int palestranteId, RedeSocialDto[] models)
        {
            try
            {
                var redesSocial = await _redeSocialPersist.GetAllByPalestranteIdAsync(palestranteId);
                if (redesSocial == null) return null;
        ...
        public async Task<bool> DeleteRedeSocialByEvento(int eventoId, int redeSocialId)
        {
            try
            {
                ...
                if (redeSocial == null) throw new Exception("Rede Social do Evento para delete não encontrada.");
        ...
        public async Task<bool> DeleteRedeSocialByPalestrante(int palestranteId, int redeSocialId)
        {
            try
            {
                ...
                if (redeSocial == null) throw new Exception("Rede Social do Palestrante para delete não encontrada.");
